...
#### `debug?: boolean` (опциональное)

При `true` библиотека пишет отладочные сообщения: события жизненного цикла (install/activate), входящие сообщения (type, sourceId), рассылку клиентам через `notifyClients` (тип, число клиентов) и исход fetch (passthrough, ответ плагина, сеть, 503). Используется `logger` (или `console`) на уровне `debug`. По умолчанию выключено; включайте при разработке.
...
**Проблема.** Когда плагин выполняет внутреннюю `fetch()` — например, `staleWhileRevalidate` извлекает свежую копию для обновления кэша, или плагин analytics отправляет событие — этот запрос может быть обработан конвейером плагина снова. Не имея возможности идентифицировать внутренние запросы, плагины могут непреднамеренно повторно применять кэширование, аналитику или другие методы работы к запросам, которые они сами инициировали. В некоторых случаях это может привести к рекурсивному запуску плагина или неправильному использованию кэша.
...
- массив плагинов нормализуется (один уровень вложенности): можно передавать результат фабрики-массива без спреда, например `[offlineFirst(options), claim()]`
