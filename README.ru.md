...
- **Предсказуемый порядок** — сначала плагины с отрицательным `order` (по возрастанию), затем без `order`, затем с неотрицательным `order` (по возрастанию)
...
- плагины сортируются: с `order` &lt; 0 (по возрастанию), затем без `order` (в порядке добавления), затем с `order` ≥ 0 (по возрастанию)
...
1. **Плагины с отрицательным `order`** — по возрастанию (например -10 → -1)
...
const plugins = [
    { name: 'fifth', order: 4 },
    { name: 'second' }, // без order
    { name: 'first', order: -1 }, // отрицательный — выполняется первым
    { name: 'fourth', order: 3 },
    { name: 'third', order: 0 },
];

// Порядок выполнения: first (order -1) → second (без order) → third (0) → fourth (3) → fifth (4)
```

**Преимущества системы:**

- 🎯 **Предсказуемость** — отрицательные → без order → неотрицательные, внутри групп сортировка по значению
- 🔧 **Простота** — отрицательный order для «ранних», положительный для «поздних», без order для «середины»
- 📈 **Масштабируемость** — легко вставлять плагины без перенумерации
