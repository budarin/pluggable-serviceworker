...
- [Опции initServiceWorker](#️-опции-initserviceworker-version-pingpath-logger-onerror)
...
- **Предсказуемый порядок** — плагины сортируются по `order` (по возрастанию, по умолчанию 0)
- **Гибкость** — можно контролировать последовательность инициализации
- **Параллельно** для `install`, `activate`, `message`, `sync`, `periodicsync`, `backgroundfetchsuccess`, `backgroundfetchfail`, `backgroundfetchabort`, `backgroundfetchclick` — независимые задачи выполняются одновременно
- **Последовательно** для `fetch` — первый вернувший ответ завершает цепочку; для `push` вызываются все плагины
- **Обработчики событий регистрируются только если есть хотя бы один плагин для этого события** — пустых слушателей нет
...
- Клиентские утилиты для работы с SW: регистрация с обходом бага `claim()`, обнаружение новой версии, подписка на сообщения, запрос версии SW, отправка сообщений, ping для "пробуждения" SW, проверка поддержки, **Background Fetch** (запуск/отмена/статус загрузок).
...
Все примитивы — **фабрики плагинов**: конфиг (если есть) передаётся при вызове по месту использования; в `options` в `initServiceWorker` попадают только `version` (обязательно), `pingPath?`, `logger?` и `onError?`. Используйте `order` в конфиге плагина для управления порядком выполнения.
...
| `notifyClients(messageType, data?)`                             | SW               | Отправляет `{ type: messageType }` или `{ type: messageType, ...data }` всем окнам-клиентам. Второй аргумент — опциональный объект с данными, поля которого добавляются в сообщение.                                   |
