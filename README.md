...
Каждый плагин реализует интерфейс `ServiceWorkerPlugin<C>`, где `C extends PluginContext` — тип контекста, который плагин ожидает. Во все обработчики вторым аргументом передаётся контекст (те же данные, что в `options` при инициализации, без `onError`).
