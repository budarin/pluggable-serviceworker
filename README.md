...
Библиотека **не вызывает** `skipWaiting()` и `clients.claim()` — это поведение задаётся индивидуально в каждом проекте и оставлено на усмотрение плагинов. При необходимости вызывайте их в своих обработчиках `install` и `activate` (в библиотеке реализованы примитивы **skipWaiting**, **claim**, **claimAndReloadClients** — смотри таблицу примитивов ниже).

**claim** — только забирает клиентов под контроль. **claimAndReloadClients** — сначала `clients.claim()`, затем перезагрузка всех окон через `client.navigate(client.url)` (порядок гарантирован внутри одного плагина). Удобно при обновлении SW со splash: после активации все вкладки получают свежую версию без кода на странице.
...
| **claimAndReloadClients** | activate | Сначала `clients.claim()`, затем перезагрузка всех окон-клиентов через `client.navigate(client.url)`. Порядок гарантирован (claim, затем reload). Подходит для обновления SW со splash: после активации все вкладки получают свежую версию. |
