...
window.addEventListener('focus', async () => {
    ...
    if (result === 'ok') {
        // SW "проснулся" — можно безопаснее использовать postMessageToServiceWorker, getServiceWorkerVersion и др.
    } else {
        // 'no-sw' или 'error' — либо SW не поддерживается, либо есть проблемы с сетью/окружением
    }
});
```
...
### 📝 Примечание про обход бага Chrome (`clients.claim()` на первом визите)

Утилита `registerServiceWorkerWithClaimWorkaround` и связанные с ней примеры предназначены для обхода бага Chrome, описанного в issue [`https://issues.chromium.org/issues/482903583`](https://issues.chromium.org/issues/482903583). Как только этот баг будет окончательно исправлен и изменение широко доедет до стабильных версий браузеров, имеет смысл:
