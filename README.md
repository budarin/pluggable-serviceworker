...
- **Последовательно** для `fetch` — первый успешный результат прерывает цепочку; для `push` — вызываются все плагины, показывается уведомление по каждому payload (или одно от библиотеки, если все вернули `undefined`)
...
- **`push`**: Возвращает `PushNotificationPayload` (объект для [Notification API](https://developer.mozilla.org/en-US/docs/Web/API/Notification)), `false` (не отображать уведомление) или `undefined` (решение об отображении отдаётся библиотеке библиотеки). Вызываются все плагины с `push`. Для каждого возврата типа `PushNotificationPayload` вызывается `showNotification` с полным набором полей. Уведомление не показывается только если **все** плагины вернули `false`. Если все вернули `undefined` - библиотека отображает уведомление.
...
- **push**: Плагин может вернуть `PushNotificationPayload`, `false` (не показывать от этого плагина) или `undefined` (решение об отображении отдаётся библиотеке). Библиотека вызывает `showNotification` для каждого возвращённого `PushNotificationPayload`. Если все плагины вернули `false`, уведомление не показывается. Если все вернули `undefined`, библиотека показывает уведомление.
